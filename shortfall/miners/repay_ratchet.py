...
class RepayRatchetShortfallMinerState(BaseMinerState):
    ...
    def summary(self):
        ...
        if self.pledge_required > 0:
            shortfall_pct = 100. * shortfall / self.pledge_required
        summary = super().summary()
        summary.update({
            'pledge_required': self.pledge_required,
            'shortfall': shortfall,
    ...
    def activate_sectors(self, net: NetworkState, power: float, duration: float, lock: float = float("inf")):
        ...
        incremental_shortfall = self._max_repayment_reward_fraction * net.expected_reward_for_power(
            power, min(duration, self._max_repayment_term))
